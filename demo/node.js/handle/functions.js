...
var images = require("images"); //图片处理模块，MakeThumb用
 ...
 *@ kind: 生成种类。1-纯数字 2-纯大写字母 3-纯小写字母 4-数字+大写字母 5-数字+小写字母 6-大写字母或小写字母 7-数字+纯大写字母或小写字母
 */
exports.CreateRandomStr = function(n, kind) {
    var _i = 0;
    var _str = "";
    ...
    switch (parseInt(kind)) {
        ...
        case 2:
            for (_i; _i < n; _i++) {
                ...
                _str += String.fromCharCode(64 + _r);
            }
            break;
        case 3:
            for (_i; _i < n; _i++) {
                _r = Math.floor(Math.random() * 26) + 1;
                _str += String.fromCharCode(96 + _r);
            }
            break;
        case 4:
            for (_i; _i < n / 2; _i++) {
                _r = Math.floor(Math.random() * 26) + 1;
                _str += Math.floor(Math.random() * 10) + String.fromCharCode(64 + _r);
            }
            break;
        case 5:
            for (_i; _i < n / 2; _i++) {
                _r = Math.floor(Math.random() * 26) + 1;
                _str += Math.floor(Math.random() * 10) + String.fromCharCode(96 + _r);
            }
            break;
        case 6:
            for (_i; _i < n; _i++) {
...
exports.DoREST = function(Host, Port, Path, Method, PostData, CallBackSuccess, CallBackError) {
    ...
    _options = { //参数集
        host: Host, //域名
        port: Port, //端口
        path: Path, //路径 www.twedding.cn/test/test.ashx，如需经GET传递数据，可写'/test/test.ashx?k=1&p=2&ps=15'
    ...
    _req = http.request(_options, function(_res) {
